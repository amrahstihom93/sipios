...
SIUPDATE=si_update
SIPI=sipios
SIPI_PATH=$PIHOME/$SIUPDATE/$SIPI
...
SCRATCH=Scratch_GUI
SCRATCH_PATH=$PIHOME/$SGMW/$SCRATCH
...
# Check if file 'Version' exists in ~/Sgmw.  If it does, move on.  If it doesn't, copy it in.
...
if file_exists $SGMW_PATH/Version 
then
   sudo cp $SIPI_PATH/Version $SGMW_PATH  # Copy version to the Dexter folder
   feedback "Copying Version file to ~/Sgmw"
else
   feedback "Version file exists, doing nothing!"
fi

sudo chmod ugo+w $SGMW_PATH/Version

echo "#############"  >>  $SGMW_PATH/Version
echo "Start: `date`"  >>  $SGMW_PATH/Version
...
geany_setup(){
  ...
  create_folder $SGMW_PATH/tmp
  sudo chmod 777 $SGMW_PATH/tmp
  ...
  if ! replace_first_this_with_that_in_file "EX_00_WD=.*" "EX_00_WD=/home/pi/Sgmw/tmp" "$GEANY_PYTHON"
  then
      add_line_to_end_of_file "EX_00_WD=/home/pi/Sgmw/tmp" "/home/pi/.config/geany/filedefs/filetypes.python"
  fi
...
sudo bash $SIPI_PATH/lib/install.sh
...
sudo cp $SIPI_PATH/script/avahi-daemon.conf /etc/avahi 		# Copy new Avahi Config File.
...
#feedback " "
# Install Scratch GUI
#sudo bash $SIPI_PATH//Scratch_GUI/install_scratch_start.sh

#feedback "--> Install Troubleshooting"
#feedback "--> ======================================="
#feedback " "
#sudo bash $SIPI_PATH//Troubleshooting_GUI/install_troubleshooting_start.sh



# Update background image - Change to dilogo.png
# These commands don't work:  sudo rm /etc/alternatives/desktop-background  ;;  sudo cp /home/pi/si_update/sipios/sigmaway_logo.jpg /etc/alternatives/
feedback "--> Update the background image on LXE Desktop."
feedback "--> ======================================="
feedback " "
sudo rm /usr/share/raspberrypi-artwork/raspberry-pi-logo-small.png
sudo cp $SIPI_PATH/sigmaway_logo.png /usr/share/raspberrypi-artwork/raspberry-pi-logo-small.png


########################################################################
## These Changes to the image are all optional.  Some users may not want
## these changes.  They should be offered a kame-hame-ha.

#sudo chmod +x $SIPI_PATH/script/wifi/wifi_disable_sleep.sh
#sudo bash $SIPI_PATH/script/wifi/wifi_disable_sleep.sh
...
########################################################################
#sudo bash $SIPI_PATH/script/upd_scratch_softlinks.sh
...
feedback "--> ======================================="
feedback " "
sudo chmod +x $SIPI_PATH/script/update_host_name.sh		# 1 - Run update_host_name.sh
sudo bash $SIPI_PATH/script/update_host_name.sh			# 2 - Add change to rc.local to new rc.local that checks for hostname on bootup.
...
feedback "--> ======================================="
feedback " "
sudo chmod +x $RASPBIAN_PATH/script/install_samba.sh
sudo bash $RASPBIAN_PATH/script/install_samba.sh
...
feedback "--> ======================================="
feedback " "
sudo chmod +x $SIPI_PATH/backup/backup.sh
sudo chmod +x $SIPI_PATH/backup/restore.sh
sudo chmod +x $SIPI_PATH/backup/call_backup.sh
sudo chmod +x $SIPI_PATH/backup/call_restore.sh
sudo chmod +x $SIPI_PATH/backup/run_backup.sh
sudo chmod +x $SIPI_PATH/backup/file_list.txt
sudo chmod +x $SIPI_PATH/backup/backup_gui.py
...
########################################################################
## Mark the start time in the Version File of the update.  
echo "End: `date`"  >>  $SGMW_PATH/Version

cd $DESKTOP_PATH
rm Version			# Delete the older versions.
rm version.desktop	# Delete the older version. 
sudo cp $SIPI_PATH/desktop/version.desktop $DESKTOP_PATH	# Copy the shortcut to the desktop
...
if [ $VERSION -eq '8' ]; then
  ...
  sudo sed -i 's/Wheezy/Jessie/g' $SGMW_PATH/Version
fi
...
if [ ! -f /home/pi/cinch ]; then
    ...
    echo "Cinch Installed."  >> $SGMW_PATH/Version
fi
